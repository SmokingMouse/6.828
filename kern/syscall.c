	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	pte_t* pte;
	//check env exist or has permission.
	if(envid2env(srcenvid,&src_env,1) < 0) return -E_BAD_ENV;
	if(envid2env(dstenvid,&dst_env,1) < 0) return -E_BAD_ENV;

	//check va legal.
	if((uint32_t)srcva >= UTOP || (uint32_t)dstva >= UTOP) return -E_INVAL;
	if((uint32_t)srcva % PGSIZE || (uint32_t)dstva % PGSIZE) return -E_INVAL;

	//check if src va exist.
	if((pg_info = page_lookup(src_env->env_pgdir,srcva,&pte)) == NULL) return -E_INVAL;
	...
	return page_insert(dst_env->env_pgdir,pg_info,dstva,perm);
	// LAB 4: Your code here.
}
